...
internal static class AppText
{
    ...
    private static readonly Dictionary<string, string> English = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Overview"] = "Overview",
        ["MainTitle"] = "AudioMirror",
        ["MainDescription"] = "Mirror one Windows playback device to multiple speakers or monitors.",
        ["RoutingTitle"] = "Audio routing",
        ["RoutingDescription"] = "Choose the Windows output to capture and the real devices that should play it.",
        ["ActionsTitle"] = "Controls",
        ["FormatTitle"] = "Format and volume",
        ...
        ["EndpointMuted"] = "{0} is muted",
        ["EndpointVolumeZero"] = "{0} volume is 0%",
        ["EndpointVolumeLow"] = "{0} volume is {1}%",
        ["EndpointVolumeWarning"] = "Volume warning: {0}.",
        ["FixVolume"] = "Unmute / 100%",
        ["FixVolumeApplied"] = "Selected devices were unmuted and set to 100%.",
        ["FixVolumeFailed"] = "Could not change device volume: {0}",
        ...
        ["Next"] = "Next",
        ["Finish"] = "Finish",
        ["NoAudioReachingSource"] = "Source is silent. This is normal if nothing is playing; if sound is playing, set Source as the Windows default output and restart the browser or player.",
        ["StatusRunningNormally"] = "Running normally",
        ["StatusNoSourceAudio"] = "Source silent",
        ["StatusWaitingDevices"] = "Waiting for devices",
        ["StatusDroppingFrames"] = "Target buffer dropping frames",
        ...
        ["TooltipFixVolume"] = "Unmutes selected Source/Targets and sets their Windows endpoint volume to 100%.",
    ...
    {
        ["en"] = English,
        ["ru"] = Merge(("Language", "Язык"), ("File", "Файл"), ("Actions", "Действия"), ("Help", "Справка"), ("Start", "Старт"), ("Stop", "Стоп"), ("Save", "Сохранить"), ("SaveSettings", "Сохранить настройки"), ("Autostart", "Автозапуск"), ("Exit", "Выход"), ("Refresh", "Обновить"), ("RefreshDevices", "Обновить устройства"), ("Sound", "Звук"), ("SoundSettings", "Параметры звука"), ("Sync", "Синхр."), ("Test", "Тест"), ("TestSpeakers", "Тест колонок"), ("UserHelp", "Справка"), ("AboutAudioMirror", "О AudioMirror"), ("OpenAudioMirror", "Открыть AudioMirror"), ("Overview", "Обзор"), ("MainTitle", "AudioMirror"), ("MainDescription", "Зеркалирует один вывод Windows на несколько колонок или мониторов."), ("RoutingTitle", "Маршрутизация звука"), ("RoutingDescription", "Выберите вывод Windows для захвата и реальные устройства, которые должны играть."), ("ActionsTitle", "Управление"), ("FormatTitle", "Формат и громкость"), ("StatusTitle", "Состояние"), ("Source", "Источник"), ("Target1", "Цель 1"), ("Target2", "Цель 2"), ("Gain", "Усиление"), ("DelayMs", "Задержка, мс"), ("AutoRestart", "Автоперезапуск"), ("Hint", "Сделайте выбранный Source устройством вывода Windows по умолчанию; если звук играет, а captured frames = 0, перезапустите плеер."), ("Close", "Закрыть"), ("Left", "Лево"), ("Right", "Право"), ("Both", "Оба"), ("Loop", "Цикл"), ("TestTitle", "Тест AudioMirror"), ("TestHint", "Left играет Target 1, Right играет Target 2, Both играет оба выхода."), ("HelpHeading", "Справка"), ("DefaultSuffix", "по умолчанию"), ("UnpluggedSuffix", "отключено"), ("NotPresentSuffix", "нет устройства"), ("Stopped", "Остановлено."), ("ModeStereo", "Стерео-зеркало")),
    ...
    static AppText()
    {
        Add("ru",
            ...
            ("Sync", "Безопасные уровни"),
            ("SetupWizard", "Мастер настройки"),
            ("SetupWizardTitle", "Настройка AudioMirror"),
            ("WizardStepSourceTitle", "Шаг 1: выберите Source"),
            ("WizardStepSourceBody", "Подтвердите Source. Это устройство, на которое Windows отправляет звук. В параметрах звука Windows сделайте это же устройство выводом по умолчанию."),
            ("WizardStepTargetsTitle", "Шаг 2: выберите колонки"),
            ("WizardStepTargetsBody", "Targets - реальные колонки, мониторы или USB-аудиоустройства, куда AudioMirror будет выводить звук."),
            ("WizardStepTestTitle", "Шаг 3: тест и запуск"),
            ("WizardStepTestBody", "Используйте тест колонок, чтобы проверить левый, правый и третий выход. Нажмите Старт, когда маршрутизация правильная."),
            ("NoAudioReachingSource", "Source сейчас без звука. Это нормально, если ничего не играет; если звук включен, сделайте Source устройством вывода Windows по умолчанию и перезапустите браузер или плеер."),
            ("StatusNoSourceAudio", "Source без звука"),
            ("TrayHintTitle", "AudioMirror продолжает работать"),
            ("TrayHintBody", "Окно свернуто в трей. Используйте значок в трее, чтобы открыть, остановить или закрыть AudioMirror."),
            ("EndpointMuted", "{0} заглушен"),
            ("EndpointVolumeZero", "Громкость {0} = 0%"),
            ("EndpointVolumeLow", "Громкость {0} = {1}%"),
            ("EndpointVolumeWarning", "Предупреждение громкости: {0}."),
            ("FixVolume", "Вкл. / 100%"),
            ("FixVolumeApplied", "Выбранные устройства включены и выставлены на 100%."),
            ("FixVolumeFailed", "Не удалось изменить громкость устройства: {0}"),
            ("TooltipFixVolume", "Включает выбранные устройства и выставляет громкость Windows на 100%."),
            ("HelpBody", "Быстрая настройка\n\n1. Сделайте свободное устройство вывода устройством Windows по умолчанию.\n2. Выберите его как Source в AudioMirror.\n3. Выберите реальные колонки или мониторы как Target 1 и Target 2.\n4. При необходимости включите Target 3 для USB-саундбара или другого выхода.\n5. Нажмите Старт.\n\nЭлементы управления\n\nУсиление - множитель громкости. 1.0 означает без изменений.\nЗадержка, мс добавляет задержку к одному выходу, чтобы уменьшить эхо.\nSplit L/R отправляет левый канал на Target 1, а правый канал на Target 2; Target 3 остается стерео.\nБезопасные уровни снижают усиление выше 1.0 обратно до 1.0.\nАвтоперезапуск пересоздает аудиопотоки после сна, отключения экрана или сброса устройства.\nТест открывает встроенную проверку колонок.\n\nЕсли Source молчит, пока звук играет, сделайте его выводом Windows по умолчанию и перезапустите браузер или плеер."),
            ("AboutTitle", "О AudioMirror"),
            ("AboutBody", "AudioMirror\n\nНебольшая WASAPI-утилита для Windows, которая зеркалирует один аудиопоток на несколько устройств вывода с отдельным усилением, задержкой, разделением каналов и встроенным тестом колонок.\n\nCopyright (c) 2026 AudioMirror contributors\n\nРепозиторий\nhttps://github.com/rausNT/audio-mirror-windows\n\nЛицензия\nMIT License\n\nПрограмма предоставляется как есть, без каких-либо гарантий."),
